...
const inputSchema = {
  customer_ref: z
    .string()
    .optional()
    .describe('Customer reference, e.g. CUS-1001. Give this, query, or both.'),
  query: z
    .string()
    .optional()
    .describe(
...
export function registerSearchCustomerTickets(
  server: McpServer,
  ...
  server.registerTool(
    ...
    async ({ customer_ref, query, status, limit, cursor }) => {
      try {
        ...
        return toToolError(error, {
          ...
          log: ctx.log,
          toolName: SEARCH_CUSTOMER_TICKETS,
        });
      }
    },
  );
